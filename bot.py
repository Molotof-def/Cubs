...
# Обязательный канал для игровых режимов
...
# Канал спонсора за бонус +50 000 монет
...
# Сетевые параметры вебхука для Render
...
# Цитаты поддержки при сериях поражений (3+ подряд)
MOTIVATIONAL_QUOTES: List[str] = [
    ...
    "🎯 <i>«Сделай глубокий вдох. Каждый проигрыш приближает тебя к грандиозному джекпоту!»</i>"
]

# Варианты работы для команды ворк
WORK_TASKS: List[str] = [
    "доставил секретный военный груз заказчику на бронированном фургоне",
    "завершил сложный высоконагруженный фриланс-проект по разработке",
    "выиграл международный киберспортивный турнир по CS с крупным призовым",
    "нашёл редкий золотой самородок в заброшенной горной шахте",
    "продал кастомный 3D-ассет персонажа на международном маркетплейсе",
    "починил упавшую серверную стойку дата-центра прямо под DDoS-атакой",
    "провёл круглосуточный благотворительный стрим и собрал море донатов",
    "собрал топовый кастомный игровой компьютер с водяным охлаждением под заказ",
    "написал прибыльного торгового бота для криптобиржи и зафиксировал профит",
    "успешно защитил корпоративную сеть банка от спланированного взлома"
]
...
QUIZ_WORDS: List[str] = [
    ...
    "сервер", "рандом", "ставка", "рулетка", "бонус", "выигрыш"
]

# Множители ступеней кубической лесенки
...
# Полная база из 46 жёстких RP-действий
RP_ACTIONS: Dict[str, Tuple[str, str]] = {
    ...
    "уничтожить": ("стёр в порошок и полностью уничтожил", "☠️"),
    "растоптать": ("грязными берцами растоптал лицо", "🥾"),
    "размазать": ("размазал тонким слоем по асфальту", "🩸"),
    ...
    "сбросить": ("сбросил вниз с крыши небоскрёба", "🏢"),
    "разорвать": ("разорвал на куски", "💥"),
...
def get_mention(user_id: int, name: Optional[str]) -> str:
    """Генерация безопасной HTML-ссылки на пользователя."""
...
def parse_amount_string(val_str: Optional[Any], current_balance: int = 0) -> Optional[int]:
    """Надёжный парсинг ставок и синонимов ва-банка."""
    if val_str is None:
        return None
...
def parse_time_string(time_str: str) -> Optional[int]:
    """Парсер длительности для мута и бана."""
...
def format_duration(seconds: int) -> str:
    """Человекопонятное представление времени."""
    if seconds >= 86400:
        days = seconds // 86400
        return f"{days} дн."
    elif seconds >= 3600:
...
def admin_dashboard_keyboard():
    builder = InlineKeyboardBuilder()
    builder.button(text="🔄 Обновить сводку", callback_data="adm_refresh_stats")
    builder.button(text="📢 Инструкция рассылки", callback_data="adm_broadcast_help")
    builder.adjust(2)
    return builder.as_markup()
...
class Database:
    ...
    async def init(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            # Безопасное добавление полей при миграции
            ...
            # Локальные администраторы чатов
            await conn.execute("""
            ...
            # Настройки чатов (RP команды и т.д.)
            await conn.execute("""
    ...
    async def transfer_money_transaction(self, sender_id: int, recipient_id: int, amount: int, received_amount: int) -> bool:
        """Атомарный перевод средств между игроками с блокировкой строк FOR UPDATE."""
        async with self.pool.acquire() as conn:
...
# ================= 6. ФОНОВЫЕ ВОТЧЕРЫ И ТАЙМАУТЫ =================
async def ladder_timeout_watcher(user_id: int, message_obj: Message):
    """При неактивности или сбое кубика авто-сохраняет выигрыш или возвращает ставку."""
    try:
...
# ================= 8. РЕЗОЛВЕРЫ ЦЕЛЕЙ И ПРОВЕРКА СТАВОК =================
...
# ================= 9. УПРАВЛЕНИЕ ДОСТУПОМ В ЧАТ (+чат / -чат) =================
...
# ================= 11. ИГРОВЫЕ РЕЖИМЫ (КУБИК, КУБЫ, ЛЕСЕНКА) =================
...
# ================= 13. СЕКРЕТНЫЕ ЧАТЫ И МАРШРУТИЗАТОР =================
